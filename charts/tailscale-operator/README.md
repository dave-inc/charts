...
| `proxyClasses.<name>.metrics.enable` | unset | Serves Prometheus metrics at `<pod-ip>:9002/metrics` and creates a `<proxy-statefulset>-metrics` Service. Auto-sets `TS_LOCAL_ADDR_PORT`/`TS_ENABLE_METRICS`, so don't set `TS_LOCAL_ADDR_PORT` again under `env`. |
| `proxyClasses.<name>.metrics.serviceMonitor.enable` | unset | Creates a Prometheus `ServiceMonitor` for that metrics Service. Requires `metrics.enable: true` (chart fails otherwise) and the Prometheus Operator CRDs. |
